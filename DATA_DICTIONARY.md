...
```
Formula: (precipitation × avg_dam_level) / 100

Example:
  precipitation = 36.8 mm
  avg_dam_level = 71.6%
  severity = (36.8 × 71.6) / 100 = 26.35
```
...
| `Train R²` | Float | R-squared score on training set | 0-1 | Proportion variance |
| `Test R²` | Float | R-squared score on test set | 0-1 | Proportion variance |
| `Test MAE` | Float | Mean Absolute Error on test set | 0-10 | Error units |
| `Overfitting Gap` | Float | Difference between train and test R² | 0-1 | Proportion difference |
...
1. ✅ High Test R² (> 0.95)
2. ✅ Low Test RMSE (< 2.5)
3. ✅ Low Overfitting Gap (< 0.05)
4. ✅ Consistent Train/Test performance
...
# Rank by test R² (primary) and overfitting gap (secondary)
df['score'] = df['Test R²'] - (0.5 * df['Overfitting Gap'])
...
| `avg_temp` | Float | Average temperature (°C) | Yes |
| `max_temp` | Float | Maximum temperature (°C) | Yes |
| `min_temp` | Float | Minimum temperature (°C) | Yes |
...
- ✅ Minimum 36 observations (3 years) for reliable modeling
